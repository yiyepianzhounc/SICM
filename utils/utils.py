...
def load_pretrain_embed(embedding_path, max_scan_num=1000000, add_seg_vocab=False):
    ...
    if add_seg_vocab:
        max_scan_num = 12287937
    max_scan_num = min(max_scan_num, 12287937)
    ...
    with open(embedding_path, 'r', encoding='utf-8') as f:
        for i in line_iter:
            ...
            if len(items) == 2:
                ...
                print("++++longer than 201+++++, %d line is: %s\n" % (i+1, line))
                token = items[0:-200]
                token = "".join(token)  
...
def gather_indexes(sequence_tensor, positions):
    ...
    flat_offsets = flat_offsets - whole_seq_length 
    ...
    flat_sequence_tensor = sequence_tensor.contiguous().view(batch_size * seq_length, -1) 
